...
% Compiles with Overleaf or tectonic; self-contained, no external figure files.
%
% BEFORE SUBMITTING:
%   1. Optionally add a personal contact address (see the author block).
% All citations verified against their sources. Submitting unaffiliated:
% Calligo's publication policy is unknown, so the affiliation is not asserted;
% it can go on the camera-ready instead.
%=============================================================================
...
% Float placement. Three floats in a two-page paper exceeds IEEEtran's defaults
% (topnumber 2, totalnumber 3, topfraction 0.7), so LaTeX cannot place them and
% defers everything to the end of the document. Relax the limits and allow a
% column to be mostly float.
...
% Unaffiliated, by decision: Calligo's publication-review policy is unknown, so
% the affiliation is not asserted on the submission. It can be added to the
% camera-ready using the commented variant below.
...
instruction trace and predicts real-core cycle counts within 0.25\%
...
Table~\ref{tab:val} gives 14 points at worst 0.25\%. Reaching that accuracy
required fixing a decoding bug in our own tooling that had mis-sized every
compressed instruction, and which surfaced only when a newly added kernel
produced a 9\% error too large to write off: widening the benchmark set is how
latent modelling bugs come to light.
...
The interface, not the arithmetic, sets the granularity. We
...
zero cycles: 13{,}321 versus 13{,}321. The offload costs a cycle,
...
Some cycles are free. Branch-target misalignment costs
...
single loop. The result we did not anticipate is that the interface, not the
arithmetic, sets the smallest fusion worth building.
...
The author thanks the co-authors of PARISCV (Spoorthi~S., Bharadhwaj~K.,
V.~Reddy and M.~Purnaprajna, Centre for Heterogeneous and Intelligent Processing
Systems, PES University) for the original work on which this builds.
